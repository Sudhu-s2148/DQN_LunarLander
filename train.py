...
save_path = f"best_network{session}.pth"
#X_pos, Y_pos, X_vel, Y_vel,theta, omega, left_contact, right_contact
with open(f"training_log{session}.csv", "a") as f:
    writer = csv.writer(f)
    writer.writerow(['episode', 'avg_steps', 'avg_reward', 'epsilon', 'temp','loss', 'action_counts'])
...
def bellmans_update(active_network, dormant_network, buffer_exp, gamma, device):
    ...
    with torch.no_grad():
        ...
        best_actions = active_network.forward(next_states_batch).argmax(dim=1, keepdim=True)
        max_Q = dormant_network.forward(next_states_batch).gather(1, best_actions).squeeze(1)
...
if training:
    for episode in range(max_ep):
        ...
        for j in range(max_steps):
            ...
            step_count+=1
            action = online_network.choice(state_tensor,epsilon,temp)
        ...
        if (episode + 1) % 100 == 0:
            ...
            for i in action_counts:
                action_counts[i] = action_counts[i]/100
            with open(f"training_log{session}.csv", "a") as f:
                writer = csv.writer(f)
                writer.writerow([episode,  avg_steps,avg_reward, epsilon,temp, loss,action_counts])
            torch.save({
                ...
                'epsilon': epsilon,
            }, f"checkpoint_session{session}.pt")
            ...
            print(f"Episode: {episode + 1} | Avg Steps: {total_steps / 100:.1f} | Avg Reward: {total_reward / 100:.1f} | Epsilon: {epsilon:.3f} | temperature: {temp:.3f} | Loss: {loss.item():.4f}")
            total_steps = 0
            total_reward = 0
    ...
    while not done:
        action = model.choice(torch.tensor(state_tensor, dtype=torch.float32).to(device), epsilon=0,temp = 1.0)
